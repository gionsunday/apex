...
const register = async (req, res) => {
    const {name, email, password} = req.body
       User.findOne({email}).exec((err, user) =>{
        ...
        const token = jwt.sign({name, email, password}, 'johnsundayjwtsecret', {expiresIn: '30m'})
...
const dashboard = async (req, res) => {
    ...
    const token = user.createJWT()
    res.status(StatusCodes.CREATED).json({user:{name:user.name, email:user.email, blocked:user.blocked, id:user._id},
    token })
}
...
const beforePassword = async (req, res) => {
    const {email} = req.body
       User.findOne({email}).exec((err, user) =>{
        ...
        const token = jwt.sign({email}, 'johnsundayjwtsecret', {expiresIn: '30m'})
...
module.exports ={
    register, login, dashboard, getallBlockedclient, getallclient, accountActivation, generalUpdate, updateStatusEarning, beforePassword,forgotPassword,Visitors
}
