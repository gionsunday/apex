...
window.addEventListener('load', () => {
   const availableAccs = localStorage.getItem("LogprofileDetails")
    ...
    const getstartedAmount =  localStorage.getItem('GetStartedAmount').split(",")[0]
    ...
    if(remaining >= 0 ){
        ...
        accBtn.addEventListener('click', async () =>{
            try {
                const data = await axios.post('https://apex-h7wm.onrender.com/apex/auth/updateactiveplan' , {
                    email:email,
                    activePlan:plan,
