...
window.addEventListener('load', async function () {
  ...
  try {
    ...
    const totalAvaliableBalance = Number(Ebtc + Eusdt + Ebnb + Eeth + earnings).toFixed(2) - capital.toFixed(2);
    ...
    ///BONUSES
    var totalBonuss = Number(depositeBonus) + Number(signupBonus) + Number(referalBonus)
    ...
    this.document.getElementById('referallink').value = referalLink || 'Getting Link...'
    ...
    document.getElementById('stopinvestment').addEventListener('click', async (e) =>{
      e.preventDefault()
      try {
        console.log(email)
