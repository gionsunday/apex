window.addEventListener("load", async () => {
    ...
    const data = await axios.post('https://apex-h7wm.onrender.com/apex/auth/getallclients')
    ...
    const { name, email, _id, btc, usdt, eth, bnb, activePlan, balanceInc, totalWithdraw, dailyEarnings, wallet, capital,totaldeposite, totalBalance, withdrawableBalance, totalEarnings } = client
    ...
    document.getElementById("withdrawalbalance").textContent = withdrawableBalance
    ...
    const newWithdrawableBalance = document.getElementById("new_withdrawable_balance")
    ...
    topupbtn.addEventListener("click", async (e) => {
        ...
        try {
            await axios.post('https://apex-h7wm.onrender.com/apex/auth/generalupdates', {
                email: clientEmail,
                btc: (_newbtc + btc),
                usdt: (_newusdt + usdt),
                eth: (_neweth + eth),
